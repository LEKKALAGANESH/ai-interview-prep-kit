import { buildPracticeQueue, practiceCoverage, recordPractice, CONFIDENCE_LEVELS, type Confidence } from "@trao/interview-prep-shared/practice.js";
...
export async function handlePractice(request:Request,store:KitStore,kitId:string):Promise<Response>{
 ...
 const body=raw as {question_id:unknown;confidence:unknown};if(typeof body.question_id!=="string"||!CONFIDENCE_LEVELS.includes(body.confidence as Confidence))return json({error:{code:"VALIDATION_ERROR",message:"Invalid question_id or confidence"}},400);
 try{const state=await store.withRequestLock(kitId,async()=>{const current=await store.getPractice(kitId);return store.savePractice(kitId,recordPractice(kit,current,body.question_id,body.confidence as Confidence));});const queue=buildPracticeQueue(kit,state);return json({state,queue,coverage:practiceCoverage(kit,state)});}catch(error){return json({error:{code:"PRACTICE_FAILED",message:error instanceof Error?error.message:"Practice update failed"}},422)}
}
